...
<head>
    ...
    <title>AI Reports - Dashboard</title>
    ...
    <style>
        /* Report Tabs — inside data-section */
        .report-tabs { display: flex; border-bottom: 2px solid var(--border-color); background: var(--bg-card); padding: 0 16px; gap: 0; overflow-x: auto; }
        .report-tab { padding: 13px 18px; border: none; background: transparent; color: var(--text-muted); font-size: 13px; font-weight: 600; cursor: pointer; border-bottom: 3px solid transparent; transition: all 0.3s; font-family: inherit; display: flex; align-items: center; gap: 7px; white-space: nowrap; }
        ...
        /* AI Response Card */
        .ai-response-card { background: var(--bg-card); border-radius: 4px; border-left: 4px solid var(--navy-accent); box-shadow: 0 2px 8px var(--shadow-color); overflow: hidden; margin: 20px 0; }
        .ai-response-header { display: flex; justify-content: space-between; align-items: center; padding: 12px 20px; background: linear-gradient(135deg, rgba(0,31,63,0.06), rgba(0,116,217,0.04)); border-bottom: 1px solid var(--border-light); font-weight: 600; font-size: 14px; color: var(--text-primary); }
        .ai-response-body { padding: 28px; line-height: 1.75; color: var(--text-primary); font-size: 14px; }
        .ai-response-body h1 { font-size: 20px; color: var(--navy-primary); margin: 20px 0 10px; padding-bottom: 6px; border-bottom: 2px solid var(--navy-accent); }
        .ai-response-body h2 { font-size: 16px; color: var(--navy-primary); margin: 18px 0 8px; padding-bottom: 4px; border-bottom: 1px solid var(--border-light); }
        .ai-response-body h3 { font-size: 14px; color: var(--text-primary); margin: 14px 0 6px; font-weight: 700; }
        ...
        .ai-response-body strong { color: var(--navy-primary); }
        .ai-response-body em { color: var(--text-secondary); }

        /* Tables inside AI response */
        .ai-response-body table, .ai-response-body .ai-table { width: 100%; border-collapse: collapse; margin: 14px 0 18px; font-size: 12px; border: 1px solid var(--border-color); border-radius: 4px; overflow: hidden; }
        .ai-response-body table th, .ai-response-body .ai-table th { background: var(--navy-primary); color: white; padding: 9px 12px; text-align: left; font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.4px; border: 1px solid rgba(255,255,255,0.1); }
        .ai-response-body table td, .ai-response-body .ai-table td { padding: 8px 12px; border: 1px solid var(--border-color); font-size: 12px; color: var(--text-primary); }
        .ai-response-body table tbody tr:nth-child(even), .ai-response-body .ai-table tbody tr:nth-child(even) { background: rgba(0,31,63,0.02); }
        .ai-response-body table tbody tr:hover, .ai-response-body .ai-table tbody tr:hover { background: rgba(0,116,217,0.06); }

        .ai-response-footer { display: flex; justify-content: space-between; align-items: center; padding: 10px 20px; font-size: 11px; color: var(--text-muted); border-top: 1px solid var(--border-light); background: rgba(0,0,0,0.01); }
        .ai-model-badge { background: rgba(0,116,217,0.1); color: var(--navy-accent); padding: 3px 10px; border-radius: 12px; font-size: 11px; font-weight: 600; }
        .ai-status-badge { display: inline-flex; align-items: center; gap: 6px; background: rgba(52,168,83,0.08); padding: 6px 14px; border-radius: 20px; font-size: 12px; font-weight: 600; color: var(--success); border: 1px solid rgba(52,168,83,0.2); }

        /* Report empty state */
        #reportArea { min-height: 200px; }
...
<body>
    ...
    <div class="app-container">
        ...
        <div class="main-content">
            <div class="header">
                <h1><i class="fas fa-brain"></i> AI Reports</h1>
                <div>Welcome, <?php echo htmlspecialchars($username); ?></div>
            </div>

            <!-- Data Section — matching project pattern -->
            <div class="data-section">
                <div class="section-header">
                    <h2><i class="fas fa-wand-magic-sparkles"></i> AI-Powered Analysis</h2>
                    <div style="display:flex;gap:10px;align-items:center;">
                        ...
                        <button class="btn btn-primary" onclick="generateReport()" style="">
                            <i class="fas fa-wand-magic-sparkles"></i> Generate Report
                        </button>
                    </div>
                </div>

                <!-- Filters Section — matching project filters-section pattern -->
                <div class="filters-section">
                    <div class="filters-header">
                        <h3><i class="fas fa-calendar-days"></i> Report Period</h3>
                        <div style="display:flex;gap:6px;">
                            <button class="btn btn-secondary btn-sm" onclick="setQuickRange('month')">This Month</button>
                            <button class="btn btn-secondary btn-sm" onclick="setQuickRange('quarter')">Quarter</button>
                            <button class="btn btn-secondary btn-sm" onclick="setQuickRange('season')">Full Season</button>
                        </div>
                    </div>
                    <div class="filters-grid">
                        <div class="filter-group">
                            <label><i class="fas fa-calendar-alt"></i> Date From</label>
                            <input type="date" id="dateFrom" value="<?php echo date('Y-m-01'); ?>" class="filter-input">
                        </div>
                        <div class="filter-group">
                            <label><i class="fas fa-calendar-alt"></i> Date To</label>
                            <input type="date" id="dateTo" value="<?php echo date('Y-m-t'); ?>" class="filter-input">
                        </div>
                    </div>
                </div>
                ...
                <div id="reportArea">
                    <div style="text-align:center;padding:50px 20px;">
                        <div style="width:80px;height:80px;margin:0 auto 16px;border-radius:50%;background:linear-gradient(135deg,rgba(0,116,217,0.1),rgba(0,116,217,0.1));display:flex;align-items:center;justify-content:center;">
                            <i class="fas fa-brain" style="font-size:32px;color:var(--navy-accent);"></i>
                        </div>
                        <h3 style="color:var(--text-primary);font-size:16px;margin-bottom:6px;">Generate AI Report</h3>
                        <p style="color:var(--text-muted);font-size:13px;max-width:400px;margin:0 auto 20px;">Select a report type, set your date range, and click Generate to create an AI-powered business analysis.</p>
